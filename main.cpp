...
int main(int argc, char* argv[]) {
    ...
    try {
        if (argc > 3 && option_exists(argv, argv + argc, "-d")) {
            ...
            if (verbose) std::cout << "Writing to output file: " << argv[3] << std::endl;
            write_file_contents(argv[3], pka2xml::decrypt_pka(input));
            if (verbose) std::cout << "Successfully decrypted file" << std::endl;
        }
            ...
            if (verbose) std::cout << "Successfully encrypted file" << std::endl;
        }
            ...
            if (verbose) std::cout << "Creating forged authentication file: " << argv[2] << std::endl;
            write_file_contents(argv[2], pka2xml::encrypt_nets("foobar~foobar~foobar~foobar~1700000000"));
            if (verbose) std::cout << "Successfully created forged file" << std::endl;
        }
            ...
            if (verbose) std::cout << "Successfully fixed file" << std::endl;
        }
            ...
            try {
                ...
                if (verbose) std::cout << "Modifying user profile name to: " << argv[3] << std::endl;
                ...
                if (verbose) std::cout << "Encrypting and writing to new file: " << new_filename << std::endl;
            ...
            if (verbose) std::cout << "Batch processing with new name: " << new_name << std::endl;
            ...
            for (int i = name_index + 1; i < argc; i++) {
                try {
                    ...
                    if (!std::filesystem::exists(input_path)) {
                        std::cerr << "Warning: Input file does not exist: " << argv[i] << std::endl;
                        fail_count++;
                        continue;
                    }
                    ...
                    if (verbose) std::cout << "\nProcessing file " << (i-name_index) << "/" << (argc-name_index-1) << ": " << argv[i] << std::endl;
                    ...
                    if (xml.empty()) {
                        std::cerr << "Error: Failed to decrypt file: " << argv[i] << std::endl;
                        fail_count++;
                        continue;
                    }
                    ...
                    if (xml.empty()) {
                        std::cerr << "Error: Failed to modify user profile name in file: " << argv[i] << std::endl;
                        fail_count++;
                        continue;
                    }
                    ...
                    std::cerr << "Error processing file " << argv[i] << ": " << e.what() << std::endl;
                    fail_count++;
                }
            }
